...
We used the R statistical programming language [@R_ref] for our analysis, using the tidyverse package [@tidy_ref] to clean the data and the lme4 package [@lme4_ref] to fit the mixed-effects model. We used the lmerTest package [@lmerTest_ref] to perform hypothesis tests on the fitted mixed-effects models.
...
In our final model, the estimated slope coefficients for concentration to be 1.8892 higher for the treatment than for the placebo. This indicates that we expect, for instance for the cell type CT101 that, in general, an increase in 1 $\mu$g/ml of concentration will lead to an increase of around 1.40 in gene expression without the treatment, and an increase of 3.29 if the treatment is applied.
...
In conclusion, we can say that treatment has a significant effect on the relationship between concentration and gene expression. 
